...
App::App(std::string title, int w, int h, int argc, char const *argv[])
{
    ...
    options.add_options()("v,vsync", "Disable V-Sync")("m,msaa", "Enable MSAA")("i,imgui", "Use Default ImGui Style")("w,width", "Window width override", cxxopts::value<int>())("h,height", "Window height override", cxxopts::value<int>())("g,gpu", "Use discrete GPU on hybrid laptops")("help", "Show Help");
    ...
    Fonts[font_cfg.Name] = io.Fonts->AddFontFromMemoryTTF(Roboto_Bold_ttf, Roboto_Bold_ttf_len, 24.0f, &font_cfg);
    ...
    Fonts[font_cfg.Name] = io.Fonts->AddFontFromMemoryTTF(Roboto_Italic_ttf, Roboto_Italic_ttf_len, 24.0f, &font_cfg);
    io.Fonts->AddFontFromMemoryTTF(fa_solid_900_ttf, fa_solid_900_ttf_len, 24.0f, &icons_config, fa_ranges);

    ImStrncpy(font_cfg.Name, "Roboto Regular", 40);
    Fonts[font_cfg.Name] = io.Fonts->AddFontFromMemoryTTF(Roboto_Regular_ttf, Roboto_Regular_ttf_len, 24.0f, &font_cfg);
    io.Fonts->AddFontFromMemoryTTF(fa_solid_900_ttf, fa_solid_900_ttf_len, 24.0f, &icons_config, fa_ranges);

    ImStrncpy(font_cfg.Name, "Roboto Mono Bold", 40);
    Fonts[font_cfg.Name] = io.Fonts->AddFontFromMemoryTTF(RobotoMono_Bold_ttf, RobotoMono_Bold_ttf_len, 48.0f, &font_cfg);
    io.Fonts->AddFontFromMemoryTTF(fa_solid_900_ttf, fa_solid_900_ttf_len, 32.0f, &icons_config, fa_ranges);

    ImStrncpy(font_cfg.Name, "Roboto Mono Italic", 40);
    Fonts[font_cfg.Name] = io.Fonts->AddFontFromMemoryTTF(RobotoMono_Italic_ttf, RobotoMono_Italic_ttf_len, 24.0f, &font_cfg);
    io.Fonts->AddFontFromMemoryTTF(fa_solid_900_ttf, fa_solid_900_ttf_len, 14.0f, &icons_config, fa_ranges);

    ImStrncpy(font_cfg.Name, "Roboto Mono Regular", 40);
    Fonts[font_cfg.Name] = io.Fonts->AddFontFromMemoryTTF(RobotoMono_Regular_ttf, RobotoMono_Regular_ttf_len, 24.0f, &font_cfg);
    io.Fonts->AddFontFromMemoryTTF(fa_solid_900_ttf, fa_solid_900_ttf_len, 14.0f, &icons_config, fa_ranges);
}
